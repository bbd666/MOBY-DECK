...
uses
  ...
  IdDiscardUDPServer, idglobal, IdAntiFreezeBase, IdAntiFreeze,idsockethandle; //,VectorGeometry;
...
type TForm1 = class(TForm)
    ...
    ChkRampTang: TCheckBox;
    ChkRampRoul: TCheckBox;
...
type TpaBasUdp = record IMAX,ind     : integer;           // index max et index courant
                        moy          : TParc;
                        tab          : array[0..199] of TParc; end;
type TFiltrUdp = record IMAX,ind     : integer;           // index max et index courant
                        moy          : TParc;
                        tab          : array[0..19] of TParc; end;
...
var
  ...
  passBasUdp: TpaBasUdp;                //
  filtreUdp : TFiltrUdp;                // buffer cyclique socket UDP
  ...
  queryDeb,queryFreq : Int64;            // pour queryPerformanceCounter
  ...
  the,phi,dz                                         :single;
  L1,L2,L3,mnv,biel,m_base,ixx_base,iyy_base,izz_base:single;
...
return y}
//==============================================================================
function maxi(l1,l2:single):single;
begin
  maxi:=i2f(l1>l2,l1,l2);
end;
//==============================================================================
...
//==============================================================================
// optimisé pour buffer cyclique de taille fixe
{procedure passBasTempoTempsReel(nbL:integer;f:double);  // f: fréq échantillonnage
var moy : TParc;
    qDeb,qFin:Int64;
begin
queryPerformanceCounter(qDeb);
  with filtreUdp do
  begin


    moy.tang:=udpParc.tang+                                       // Moyenne glissante 4 points dans buffer cyclique
      tab[I2F( ind =0           ,IMAX    -1,ind-1)].tang+       // avec gestion du saut d'indice
      tab[I2F((ind>=0)and(ind<2),IMAX-ind-1,ind-2)].tang+       // pour une loi de pondération chronologique
      tab[I2F((ind>=0)and(ind<3),IMAX-ind-1,ind-3)].tang)/4;
  end;
queryPerformanceCounter(qFin);
Form1.LblFiltreUdp.Caption:=floatToStrf(1000000*(qFin-qDeb-550)/queryFreq,fffixed,5,1)+' µs filtre';Form1.LblFiltreUdp.Refresh;
end;}
//==============================================================================
...
toutVaBien:                                                    // quand tout se passe bien
   if csgMot[iMot].min>tens then
   begin
     csgMot[iMot].min:=tens;                                   // mémo du mini => <-10V
     form1.LblUminMotG.Caption :=floatToStrf(tens,fffixed,5,1)+' V';
   end;
   if csgMot[iMot].max<tens then
   begin
     csgMot[iMot].max:=tens;                                   // mémo du maxi => >+10V
     form1.LblUmaxMotG.Caption:=floatToStrf(tens,fffixed,5,1)+' V';
   end;
   if tens+uOffVar[iMot]> 10 then tens:= 10-uOffVar[iMot];     // +10V maxi sinon sortie=0V
   if tens+uOffVar[iMot]<-10 then tens:=-10-uOffVar[iMot];     // -10V mini sinon sortie=0V
   _8136_A_Write_Volt(CARD0,CH3+iMot,tens+uOffVar[iMot]);      // de -10V à +10V
end;
//==============================================================================
...
//==============================================================================
procedure TForm1.RSTimerParcTimer(Sender: TObject);   // Exécute le parcours
begin
  ...
  iParc:=iParc+1;
  if iParc=length(tabParcAng) then
  begin
...
var
  ...
  dur :double;
begin
  move(Adata[0],buff,12);                                   // 3 x 4 single = 12 octets
  with udpParc do
  try
    ...
    if (0.06-buff.position[2]>0.27) and (0.06-buff.position[2]<0.49) then pomp:=0.06-buff.position[2];
    finally
//    LblDataUdp.Caption:=floatToStrf(tang,fffixed,5,2)+' m/s²';Form1.LblDataUdp.Refresh;
  end;
  queryPerformanceCounter(qFin);
...
var poub: double;
    tg,rl :single;
begin
  with udpParc do begin tg:=tang; rl:=roul; end;
  with parConv do
  begin
    ...
    if abs(rl)+abs(tg)>angleMax then
    begin
    ...
    with passBasUdp do begin
      moy.tang:=moy.tang-(tab[(ind+1) mod IMAX].tang-tg)/IMAX; // moyenne glissante sur 200x3ms
      moy.roul:=moy.roul-(tab[(ind+1) mod IMAX].roul-rl)/IMAX;
      tab[ind].tang:=tg;
      tab[ind].roul:=rl;
//    passBasTempo(nbLig,330);                        // fréq échantillonnage 330Hz car dT=3ms
      form1.edtthe.Text:=floatToStrf(moy.tang,fffixed,5,2);   //
      form1.edtphi.Text:=floatToStrf(moy.roul,fffixed,5,2);
    end;
//    form1.edtthe.Text:=floatToStrf(tg,fffixed,5,2);   //
//    form1.edtphi.Text:=floatToStrf(rl,fffixed,5,2);
  end;
end;
//==============================================================================
procedure traiteSocketUdp();
var qFin:Int64;
begin
  with udpParc do
  begin
...
form1.caption:=floattostrf(tang,fffixed,5,3)+'   '+floattostrf(roul,fffixed,5,3)+'   '+floattostrf(pomp,fffixed,5,3);
    if (tang<>memoParc.tang) or
       (roul<>memoParc.roul) or
       (pomp<>memoParc.pomp) then // nouvelle valeur ?
    begin
      queryPerformanceCounter(qFin);
//      Form1.LblTraiteUdp.Caption:=floatToStrf(1000000*(qFin-queryDeb)/queryFreq,fffixed,5,1)+' µs traite UDP';Form1.LblTraiteUdp.Refresh;
      memoParc.tang:=tang;
      memoParc.roul:=roul;
      memoParc.pomp:=pomp;
      queryDeb:=qFin;
    ...
    if pomp> biel+mnv then pomp:= biel+mnv;                // limite haute atteinte
    if pomp<-biel-mnv then pomp:=-biel-mnv;                // limite basse atteinte
    //    passBasTempo(nbLig,330);                             // fréq échantillonnage 330Hz car dT=3ms
    with passBasUdp do begin
      moy.pomp:=moy.pomp-(tab[(ind+1) mod IMAX].pomp-pomp)/IMAX; // moyenne glissante sur 200x3ms
      tab[ind].roul:=pomp;
      ind:=(ind+1) mod IMAX;
    end;
    if pomp>biel then pomp:=pomp-0.00005;                  // descente lente vers H0, 0.05mm/3ms => 17mm/s
    if pomp<biel then pomp:=pomp+0.00005;                  // montée lente vers H0
    form1.edtdZ.Text:=floatToStrf(passBasUdp.moy.pomp,fffixed,5,3);
  end;
end;
//==============================================================================
procedure TForm1.RSTimerUdpTimer(Sender: TObject); begin traiteSocketUdp(); end;
//==============================================================================
procedure TForm1.ChkBoxUdpClick(Sender: TObject);
var id :integer;
begin
  if ChkBoxUdp.Checked then litParamConvers();              // lit paramètres de loi de mouvement
  RSTimerUdp.Enabled:=ChkBoxUdp.Checked;
  filtreUdp.IMAX:=length(filtreUdp.tab);filtreUdp.ind:=0;   // init buffer cyclique de filtrage pass-bas
  filtreUdp.moy.tang:=0;filtreUdp.moy.roul:=0;filtreUdp.moy.pomp:=0;
  with passBasUdp do begin
    IMAX:=length(tab);ind:=0;   // init buffer cyclique de filtrage pass-bas
    moy.tang:=0;moy.roul:=0;
    for id:=0 to passBasUdp.IMAX do begin tab[id].tang:=0;tab[id].roul:=0; end;
  end;  
end;
//==============================================================================
...
{ moyGlis:=strToInt(Form1.EdtMoyGlis.Text);                          // nb de points
  ...
  with tabParcAng[i] do
  begin
    poub:=0;for g:=i-moyGlis to i-1 do poub:=poub+tabParcAng[g].tang;tang:=poub/moyGlis;
    poub:=0;for g:=i-moyGlis to i-1 do poub:=poub+tabParcAng[g].roul;roul:=poub/moyGlis;
    poub:=0;for g:=i-moyGlis to i-1 do poub:=poub+tabParcAng[g].pomp;pomp:=poub/moyGlis;
  end;}
  ...
  begin                                                              // compactage angulaire
    ...
    if abs(roul)+abs(tang)>angleMax then
    begin
...
//passBasTempo( 8,50);  // pour évaluer la durée de traitement
//passBasTempo(16,50);  // pour évaluer la durée de traitement
//passBasTempoTempsReel(32,50);  // pour évaluer la durée de traitement
//passBasTempo(64,50);  // pour évaluer la durée de traitement
//passBasTempo( 20,50);  // pour évaluer la durée de traitement
//passBasTempo( 50,50);  // pour évaluer la durée de traitement
...
var tension:double;
begin
  if form1.ChkAsser.Checked then
  begin
...
{   if bRepos[3] then    // 3 moteurs au repos PF ?
  ...
  begin
    if form1.ChkEpreuve.Checked then                                   // éxécution des 3 diagrammes d'épreuve en vitesse
    begin
      ...
      if iMot=DR then// GA then                                                  // si 3éme moteur
      begin
...
begin
  ...
  begin
    ...
    for iMot:=DR to GA do
    begin
      if TcheckBox(form1.FindComponent('ChkDiagAct'+IntToStr(iMot+1))).checked then     // si ce moteur actif
      begin
...
begin
  glObj := TGLBaseSceneObject(NewtonBodyGetUserData(body)) ;
  glObj.matrix := pMatrix(matrix)^ ;
  globj.Scale.X:=0.01*mnv/0.12;
  globj.Scale.y:=0.01*mnv/0.12;
  globj.Scale.z:=0.01*mnv/0.12;
end ;
//==============================================================================================
...
var
  vPf:TVector3d;        //dVc:TVector3d;
  ang:double;           //norm:double;
...
//  dVc[0]:=vPf[0];dVc[1]:=0;dVc[2]:=vPf[2];                  // projeter dans le plan x,z
//  norm:=sqrt(dVc[0]*dVc[0])+sqrt(dVc[2]*dVc[2]);            // norme de la projection
//  dVc[0]:=dVc[0]/norm;dVc[2]:=dVc[2]/norm;                  // normer la projection
  ang:=(arctan(vPf[0]/vPf[2])*180/PI);                        // dans le plan x,z
  Form1.LblMesTang.Caption:=floattostrf(ang,fffixed,5,3); // mesure
//  Form1.LblMesTang.Caption:=floattostrf(the-ang,fffixed,5,3); // écart : consigne-mesure
  ang:=-(arctan(vPf[1]/vPf[2])*180/PI);                       // dans le plan y,z
  Form1.LblMesRoul.Caption:=floattostrf(ang,fffixed,5,3); // mesure
//  Form1.LblMesRoul.Caption:=floattostrf(phi-ang,fffixed,5,3); // écart : consigne-mesure
end;
//==============================================================================================
...
begin
  ...
  mposition0     :=Rel2abspoint (Form1.bielle   [0],affinevectormake(-0.03,  0,mnv));
...
begin
  ...
  mposition0:=Rel2abspoint(Form1.bielle   [1],affinevectormake(-0.03,0,mnv));
...
begin
  ...
  mposition0:=Rel2abspoint(Form1.bielle   [2],affinevectormake(-0.03,0,mnv));
...
begin
  ...
  mnv         :=DATA_INI.READFLOAT('GEOMETRIE' ,'manivelle',0.12);    // en m
...
begin
  ...
  begin
    ...
    b:=2*yN[iM];
    c:=sqr(xN[iM])+sqr(yN[iM])-sqr(bielV[iM])+sqr(mnv);
    delta:=sqr(2*a*c)-4*(sqr(a)+sqr(b))*(sqr(c)-sqr(b)*sqr(mnv));
...
begin
  ...
  if Form1.chkMiCourseAuto.Checked then
  begin
    pente:=((AMAX-abs(phi)/2)*(H0-HMIN)/AMAX+HMIN-H0)/(AMAX-abs(phi)/2); // voir fichier "vitesseAccélérationMoteurs.xlsx"
    dz:=(pente*the)*Form1.Tbar7.Position/100+H0;                         // avec mi-course proportionnelle
...
begin
  edtTheChge();                  // instantané
// Form1.RSTimer4.Enabled:=true;  // différé 5 ms
end;
//==============================================================================================
