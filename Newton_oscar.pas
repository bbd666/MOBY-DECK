...
begin
  ...
  msg:='';
  for i:=0 to memo1.lines.Count-1 do msg:=msg+memo1.Lines[i]+'&';
...
begin
      ...
      form2.Memo1.Lines.Add('Roulis Amp : '+CBCRAmp.items[CBCRAmp.itemindex]+' Fréq : '+CBCRFreq.items[CBCRFreq.itemindex]);
      form2.Memo1.Lines.Add('Tangage Amp : '+CBCTAmp.items[CBCTAmp.itemindex]+'Fréq : '+CBCTFreq.items[CBCTFreq.itemindex]);
...
begin
  memo1.Clear;
  if CBRAmp.ItemIndex=1 then amplitude:='moyenne' else if CBRAmp.ItemIndex=2 then amplitude:='elevee';
  if CBRFreq.ItemIndex=0 then frequence:='faible' else if CBRFreq.ItemIndex=1 then frequence:='moyenne' else
  if CBRFreq.ItemIndex=2 then frequence:='elevee' else if CBRFreq.ItemIndex=3 then frequence:='tres elevee';
  memo1.Lines.Add('~ Essai de ROULIS ~');
  memo1.Lines.Add('Amplitude '+amplitude);
  memo1.Lines.Add('Frequence '+frequence);
...
begin
  memo1.Clear;
  if CBTAmp.ItemIndex=0 then amplitude:='moyenne' else if CBTAmp.ItemIndex=1 then amplitude:='elevee';
  if CBTFreq.ItemIndex=0 then frequence:='faible' else if CBTFreq.ItemIndex=1 then frequence:='moyenne' else
  if CBTFreq.ItemIndex=2 then frequence:='elevee' else if CBTFreq.ItemIndex=3 then frequence:='tres elevee';
  memo1.Lines.Add('~ Essai de TANGAGE ~');
  memo1.Lines.Add('Amplitude '+amplitude);
  memo1.Lines.Add('Frequence '+frequence);
end;
...
begin
  memo1.Clear;
  if CBPAmp.ItemIndex=0 then amplitude:='moyenne' else if CBPAmp.ItemIndex=1 then amplitude:='elevee';
  if CBPFreq.ItemIndex=0 then frequence:='faible' else if CBPFreq.ItemIndex=1 then frequence:='moyenne' else
  if CBPFreq.ItemIndex=2 then frequence:='elevee' else if CBPFreq.ItemIndex=3 then frequence:='tres elevee';
  memo1.Lines.Add('~ Essai de MOUVEMENT VERTICAL ~');
  memo1.Lines.Add('Amplitude '+amplitude);
  memo1.Lines.Add('Frequence '+frequence);
end;
...
begin
  ...
  if CBCRAmp.ItemIndex=2 then amplitude1:='elevee' else if CBCRAmp.ItemIndex=3 then amplitude1:='tres elevee';
  if CBCRFreq.ItemIndex=0 then frequence1:='faible' else if CBCRFreq.ItemIndex=1 then frequence1:='moyenne' else
  if CBCRFreq.ItemIndex=2 then frequence1:='elevee' else if CBCRFreq.ItemIndex=3 then frequence1:='tres elevee';
  if CBCTAmp.ItemIndex=0 then amplitude2:='faible' else if CBCTAmp.ItemIndex=1 then amplitude2:='moyenne'else
  if CBCTAmp.ItemIndex=2 then amplitude2:='elevee' else if CBCTAmp.ItemIndex=3 then amplitude2:='tres elevee';
  if CBCTFreq.ItemIndex=0 then frequence2:='faible' else if CBCTFreq.ItemIndex=1 then frequence2:='moyenne' else
  if CBCTFreq.ItemIndex=2 then frequence2:='elevee' else if CBCTFreq.ItemIndex=3 then frequence2:='tres elevee';
  memo1.Lines.Add('~ Essai COMBINE : Roulis et Tangage ~');
  memo1.Lines.Add('-Roulis-');
