...
type TqMemCyc  = record IMAX,ind     : integer;
                        sdtB,seaB    : TStatRT;
...
var
  ...
  cptPomp:integer;                       // pour pompage, index moteur
...
begin
  _8136_A_Write_Volt(CARD0,CH3+iMot,tens+uOffVar[iMot]);
end;
//==============================================================================
...
begin
  strFile:='oscarAsser--';
  if iMot=0 then strFile:=strFile+'DR';
  ...
  WriteLn(myFile,'IntraD',chr(9),'InterD'  ,chr(9),'Temps(s)',chr(9),
                 'Mesure',chr(9),'Consigne',chr(9),'Ecart'   ,chr(9),'Correct.');
  with qMemCyc[iMot] do
  begin
    decal:=tTqMe[ind mod IMAX].intra.qDeb;
    for i:=0 to qMemCyc[iMot].IMAX-1 do
    begin
      j:=(i+ind) mod IMAX;
      Writeln(myFile,1000*(tTqMe[j].intra.qFin-tTqMe[j].intra.qDeb-550)/queryFreq:5:3,chr(9), // durée asserHard en ms
                     1000*(tTqMe[j].inter.qFin-tTqMe[j].inter.qDeb-550)/queryFreq:5:3,chr(9), // durée Timer en ms
                     1000*(tTqMe[j].intra.qDeb-              decal    )/queryFreq:5:3,chr(9), // base de temps pour graphique en µs
                                                                    tTqMe[j].mes :5:3,chr(9), // mesure
                                                                    tTqMe[j].csg :5:3,chr(9), // consigne
                                                       tTqMe[j].csg-tTqMe[j].mes :5:3,chr(9), // écart
                                                                    tTqMe[j].corr:5:3);       // correction
    end;                                                                
  end;
  CloseFile(myFile);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      queryPerformanceCounter(tTqMe[ind                    ].intra.qDeb);
      queryPerformanceCounter(tTqMe[iif(ind=0,IMAX-1,ind-1)].inter.qFin);
      queryPerformanceCounter(tTqMe[ind                    ].inter.qDeb);
    end;
    durT:=trunc(1000000*(tTqMe[iif(ind=0,IMAX-1,ind-1)].inter.qFin-
                         tTqMe[iif(ind=0,IMAX-1,ind-1)].inter.qDeb-550)/queryFreq);   //en µs
...
{      consMoy:=(cons[iMot]+
                ...
                tTqMe[iif(ind=0,                 IMAX-2,iif(ind=1,IMAX-1,ind-2 ))].csg+
                tTqMe[iif(ind=0,IMAX-3,iif(ind=1,IMAX-2,iif(ind=2,IMAX-1,ind-3)))].csg)/4; // Moyenne glissante 4 points
}
...
//    form1.Caption:=inttostr(sea.cpt)+'  '+inttostr(durT)+'  '+floattostrf(durT/DTIM,fffixed,6,4);
    pid[iMot].oldEc:=ecr;                                                        // Dérivée
    corr:=ecr           *strtofloat(Form1.LblKP.Caption)+                                    // Correction PID
          {pid[iMot].intg*Form1.LblKI.Caption+}
...
//    consMoy:=corr;                                                            // Consigne corrigée
    ...
    begin
      ...
      if durT div 1000>0 then calcStatTempsReel(sdt,durT/1000);
...
//    if abs(corr)<Form1.TBarKI.Position/200 then corr:=corr*corr/Form1.TBarKI.Position/200;   // zone morte => aggrave le 92Hz
    if existcard<>0 then _8136_A_Write_Varia(iMot,iif(mes=120.0,0.0,corr));                    // Sortie analogique
    if not bFile[iMot] then
    begin
      queryPerformanceCounter(tTqMe[ind].intra.qfin);
      ind:=ind+1;
    end;
  end;
end;
...
(*  for i:=0 to 2 do
  ...
  LblKP.Caption:=floattostrf(TBarKP.Position/200,fffixed,5,2);
  LblKI.Caption:=floattostrf(TBarKI.Position/200,fffixed,5,2);
  LblKD.Caption:=floattostrf(TBarKD.Position/10,fffixed,5,2);
...
//==============================================================================================
procedure TForm1.TBarKPChange(Sender: TObject); begin LblKP.Caption:=floattostrf(TBarKP.Position/200,fffixed,5,2);initStatTempsReel(sea); end;
